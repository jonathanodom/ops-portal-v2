            ...
            @forelse($closeout?->timeEntries ?? collect() as $entry)
                ...
                <article class="rounded-lg border border-slate-200 p-4 text-sm">
                    ...
                    @if($closeout?->status === 'draft' && $entry->user_id === auth()->id() && $entry->ended_at)
                        <details class="mt-2"><summary class="min-h-11 cursor-pointer py-3 font-bold text-brand-blue">Correct my entry</summary><form method="POST" action="{{ route('field.visits.time.update', [$visit, $entry]) }}" class="space-y-3">@csrf @method('PUT')<label class="form-label" for="v2_started_{{ $entry->id }}">Started</label><input class="form-input" id="v2_started_{{ $entry->id }}" name="started_at" type="datetime-local" value="{{ $entry->started_at->timezone($visit->timezone)->format('Y-m-d\TH:i') }}" required><label class="form-label" for="v2_ended_{{ $entry->id }}">Ended</label><input class="form-input" id="v2_ended_{{ $entry->id }}" name="ended_at" type="datetime-local" value="{{ $entry->ended_at->timezone($visit->timezone)->format('Y-m-d\TH:i') }}" required><label class="form-label" for="v2_reason_{{ $entry->id }}">Correction reason</label><textarea class="form-textarea" id="v2_reason_{{ $entry->id }}" name="correction_reason" required></textarea><button class="button-secondary w-full">Save correction</button></form></details>
                    @endif
                </article>
