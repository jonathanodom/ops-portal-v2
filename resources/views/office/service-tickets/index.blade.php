<x-layouts.office title="Service Tickets" width="workspace">
    ...
    <form method="GET" class="office-filter-toolbar xl:grid-cols-[minmax(280px,1.5fr)_repeat(4,minmax(140px,0.65fr))_auto]" aria-label="Service Ticket filters">
        ...
        <div class="flex flex-wrap gap-2"><button class="button-secondary">Filter</button>@if(request()->hasAny(['search','status','priority','source','assignee']))<a href="{{ route('office.service-tickets.index') }}" class="inline-flex min-h-11 items-center px-2 text-sm font-bold text-brand-blue underline">Clear</a>@endif</div>
    </form>
    ...
    <div class="office-table-wrap" data-office-table>
        <table class="office-data-table">
            ...
            <thead><tr><th scope="col">Service Ticket</th><th scope="col">Customer and location</th><th scope="col">Source</th><th scope="col">Visits</th><th scope="col">Priority</th><th scope="col">Status</th><th scope="col" class="text-right">Action</th></tr></thead>
            <tbody>
                @forelse($tickets as $ticket)
                    <tr>
                        ...
                        <td>{{ $sources[$ticket->source] ?? ucfirst($ticket->source) }}</td>
