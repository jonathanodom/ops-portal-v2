<x-layouts.office :title="$ticket->ticket_number">
    ...
    <div class="mt-2 flex flex-wrap items-start justify-between gap-4">
        ...
        <div class="flex flex-wrap gap-2"><details class="relative"><summary class="button-secondary cursor-pointer list-none">Documents</summary><div class="absolute right-0 z-20 mt-2 w-80 max-w-[calc(100vw-2rem)] rounded-lg border border-slate-200 bg-white p-2 shadow-lg"><a class="block rounded-md p-3 hover:bg-slate-50 focus-visible:outline-2 focus-visible:outline-brand-blue" href="{{ route('office.service-tickets.documents.technician-work-order', $ticket) }}"><strong>Technician Work Order</strong><span class="mt-1 block text-xs text-slate-600">Schedule, crew, scope, field notes, and paper acknowledgment.</span></a><a class="block rounded-md p-3 hover:bg-slate-50 focus-visible:outline-2 focus-visible:outline-brand-blue" href="{{ route('office.service-tickets.documents.completion-summary', $ticket) }}"><strong>Completion Summary</strong><span class="mt-1 block text-xs text-slate-600">Concise customer who, what, when, where, and outcome.</span></a><a class="block rounded-md p-3 hover:bg-slate-50 focus-visible:outline-2 focus-visible:outline-brand-blue" href="{{ route('office.service-tickets.documents.customer-service-record', $ticket) }}"><strong>Customer Service Record</strong><span class="mt-1 block text-xs text-slate-600">Customer-safe Visit and acknowledgment history.</span></a><a class="block rounded-md p-3 hover:bg-slate-50 focus-visible:outline-2 focus-visible:outline-brand-blue" href="{{ route('office.service-tickets.documents.detailed-service-report', $ticket) }}"><strong>Detailed Service Report</strong><span class="mt-1 block text-xs text-slate-600">Internal timelines, Closeouts, attribution, evidence, and authorized billing.</span></a></div></details>@if($activeMembership->hasCapability('dispatch.manage'))<a href="{{ route('office.service-tickets.edit', $ticket) }}" class="button-secondary">Edit ticket</a>@if(in_array($ticket->status, ['open', 'on_hold'], true))<a href="{{ route('office.service-tickets.visits.create', $ticket) }}" class="button-primary">Add visit</a>@endif @endif</div>
    </div>
