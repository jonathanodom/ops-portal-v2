<x-layouts.office :title="$ticket->ticket_number" width="detail">
    ...
    <x-office.record-header :title="$ticket->title" :back-href="route('office.service-tickets.index')" back-label="Service Tickets" :description="$ticket->ticket_number.' · '.($purposes[$ticket->purpose] ?? ucfirst(str_replace('_',' ',$ticket->purpose))).' · '.($billingDispositions[$ticket->billing_disposition] ?? ucfirst(str_replace('_',' ',$ticket->billing_disposition)))">
        <x-slot:badges><span class="{{ $ticket->status === 'open' ? 'status-active' : ($ticket->status === 'on_hold' ? 'status-hold' : 'status-inactive') }}">{{ ucfirst(str_replace('_',' ',$ticket->status)) }}</span><span class="{{ in_array($ticket->priority, ['high','urgent'], true) ? 'status-priority' : 'status-muted' }}">{{ ucfirst($ticket->priority) }} priority</span></x-slot:badges>
        <x-slot:actions><details class="relative"><summary class="button-secondary cursor-pointer list-none">Documents</summary><div class="absolute right-0 z-20 mt-2 w-80 max-w-[calc(100vw-2rem)] border border-slate-200 bg-white p-2"><a class="block p-3 hover:bg-slate-50 focus-visible:outline-2 focus-visible:outline-brand-blue" href="{{ route('office.service-tickets.documents.technician-work-order', $ticket) }}"><strong>Technician Work Order</strong><span class="mt-1 block text-xs text-slate-600">Schedule, crew, scope, field notes, and acknowledgment.</span></a><a class="block p-3 hover:bg-slate-50 focus-visible:outline-2 focus-visible:outline-brand-blue" href="{{ route('office.service-tickets.documents.completion-summary', $ticket) }}"><strong>Completion Summary</strong><span class="mt-1 block text-xs text-slate-600">Concise customer outcome.</span></a><a class="block p-3 hover:bg-slate-50 focus-visible:outline-2 focus-visible:outline-brand-blue" href="{{ route('office.service-tickets.documents.customer-service-record', $ticket) }}"><strong>Customer Service Record</strong><span class="mt-1 block text-xs text-slate-600">Customer-safe Visit history.</span></a><a class="block p-3 hover:bg-slate-50 focus-visible:outline-2 focus-visible:outline-brand-blue" href="{{ route('office.service-tickets.documents.detailed-service-report', $ticket) }}"><strong>Detailed Service Report</strong><span class="mt-1 block text-xs text-slate-600">Authorized internal detail.</span></a></div></details>@if($activeMembership->hasCapability('dispatch.manage'))<a href="{{ route('office.service-tickets.edit', $ticket) }}" class="button-secondary">Edit ticket</a>@if(in_array($ticket->status, ['open', 'on_hold'], true))<a href="{{ route('office.service-tickets.visits.create', $ticket) }}" class="button-primary">Add visit</a>@endif @endif</x-slot:actions>
        ...
        <div class="space-y-6">
            ...
            <section class="surface overflow-hidden">
                ...
                @forelse($ticket->visits as $visit)
                    @php($visitConfirmation = $visit->confirmationForCurrentSchedule())
                    @php($visitConfirmationState = $visit->confirmationState(now(), $activeOrganization->timezone))
                    <div class="border-b border-slate-200 p-5 last:border-0">
                        <div class="flex flex-wrap justify-between gap-3"><div><div class="flex flex-wrap items-center gap-2"><p class="font-bold">{{ $visit->displayLabel() }}</p>@if($visit->scheduled_start_at)<span class="{{ $visitConfirmationState === 'confirmed' ? 'status-success' : ($visitConfirmationState === 'needs_confirmation' ? 'status-priority' : 'status-muted') }}">{{ $visit->confirmationLabel(now(), $activeOrganization->timezone) }}</span>@endif</div><p class="mt-1 text-sm text-slate-600">{{ ucfirst(str_replace('_',' ',$visit->status)) }} · {{ $visit->scheduledStartLocal()?->format('M j, Y g:i A T') ?? 'Unscheduled' }}@if($visit->scheduledEndLocal()) – {{ $visit->scheduledEndLocal()->format('g:i A T') }}@endif</p><p class="mt-1 text-sm text-slate-500">{{ $visit->assignments->map(fn($a) => ($a->is_lead ? 'Lead: ' : '').$a->membership->user->name)->join(', ') ?: 'No crew assigned' }}</p>@if($visitConfirmation)<p class="mt-2 text-sm text-slate-600">Confirmed by {{ $visitConfirmation->confirmedBy?->name ?? 'Former user' }} via {{ Str::headline($visitConfirmation->method) }} · <x-local-time :value="$visitConfirmation->confirmed_at" :timezone="$activeOrganization->timezone" />@if($visitConfirmation->note)<br><span class="whitespace-pre-line">{{ $visitConfirmation->note }}</span>@endif</p>@endif</div><div class="flex flex-wrap gap-2">@if(in_array($visit->id, $executableVisitIds, true))<button type="button" class="button-primary" data-dialog-open="execution-visit-{{ $visit->id }}">Open execution</button>@endif @if($activeMembership->hasCapability('dispatch.manage') && in_array($ticket->status, ['open', 'on_hold'], true) && in_array($visit->status, ['planned', 'scheduled', 'assigned'], true))<a class="button-secondary" href="{{ route('office.visits.edit', $visit) }}">Schedule / assign</a>@endif</div></div>
                        @if($activeMembership->hasCapability('dispatch.manage') && $visit->scheduled_start_at && in_array($visit->status, ['scheduled', 'assigned'], true) && !$visitConfirmation)
                            <details class="mt-4 border border-slate-200 bg-slate-50 p-3" @if((int) old('confirmation_visit_id') === $visit->id) open @endif>
                                <summary class="flex min-h-11 cursor-pointer items-center font-bold text-slate-700">Record confirmation</summary>
                                <form method="POST" action="{{ route('office.visits.confirmation.store', $visit) }}" class="mt-3 grid gap-3 sm:grid-cols-2">
                                    @csrf
                                    <input type="hidden" name="confirmation_visit_id" value="{{ $visit->id }}">
                                    <div><label class="form-label" for="confirmation_method_{{ $visit->id }}">Method</label><select class="form-input" id="confirmation_method_{{ $visit->id }}" name="confirmation_method" required><option value="">Choose method</option>@foreach(['call'=>'Call','text'=>'Text','email'=>'Email','in_person'=>'In Person'] as $value=>$label)<option value="{{ $value }}" @selected(old('confirmation_method')===$value)>{{ $label }}</option>@endforeach</select><x-field-error field="confirmation_method" /></div>
                                    <div><label class="form-label" for="confirmation_note_{{ $visit->id }}">Note <span class="font-normal text-slate-500">(optional)</span></label><textarea class="form-textarea" id="confirmation_note_{{ $visit->id }}" name="confirmation_note" maxlength="2000">{{ old('confirmation_note') }}</textarea><x-field-error field="confirmation_note" /></div>
                                    <button class="button-primary sm:col-span-2">Confirm appointment</button>
                                </form>
                            </details>
                        @endif
            ...
            @php($workTimeRollup = app(\App\Domain\WorkItemTimeAttribution::class)->forTicket($ticket))
            <section class="surface p-5"><h2 class="text-lg font-bold">Work Time Attribution</h2><p class="mt-1 text-sm text-slate-600">Operational attribution only; excludes travel and does not alter billable labor.</p><div class="mt-4 space-y-2">@forelse($workTimeRollup as $row)<div class="flex min-h-11 items-center justify-between gap-3 border-b border-slate-100 py-2"><span class="min-w-0 break-words font-semibold">{{ $row['label'] }}</span><span>{{ intdiv($row['seconds'],3600) }} hr {{ intdiv($row['seconds']%3600,60) }} min</span></div>@empty<p class="text-sm text-slate-500">No on-site or other work time recorded.</p>@endforelse</div></section>
