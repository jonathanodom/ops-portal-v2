<x-layouts.office title="Edit customer" width="form">
    <x-office.record-header title="Edit customer" :back-href="route('office.customers.show', $customer)" :back-label="$customer->display_name" description="Update customer identity, status, and office-only details." />
    <x-form-errors />
    <form method="POST" action="{{ route('office.customers.update', $customer) }}" class="office-form-shell">
        @csrf @method('PUT')
        ...
        <x-office.form-actions><a href="{{ route('office.customers.show', $customer) }}" class="button-secondary">Cancel</a><button class="button-primary">Save customer</button></x-office.form-actions>
    </form>
</x-layouts.office>
