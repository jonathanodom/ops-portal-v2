    ...
    <div class="min-h-screen lg:grid lg:grid-cols-[248px_minmax(0,1fr)]">
        <aside class="hidden border-r border-slate-200 bg-white p-5 lg:flex lg:flex-col">
            ...
            <nav class="mt-3" aria-label="Office">
                ...
                @if ($activeMembership->hasCapability('projects.view'))
                    <a href="{{ route('office.projects.index') }}" @if(request()->routeIs('office.projects.*')) aria-current="page" @endif class="mt-1 flex min-h-11 items-center rounded-lg border-l-4 px-4 text-sm font-bold {{ request()->routeIs('office.projects.*') ? 'border-brand-blue bg-blue-50 text-brand-blue-dark' : 'border-transparent text-slate-600 hover:bg-slate-50' }}">Projects</a>
                @endif
                @if ($activeMembership->hasCapability('opportunities.view'))
                    <a href="{{ route('office.opportunities.index') }}" @if(request()->routeIs('office.opportunities.*')) aria-current="page" @endif class="mt-1 flex min-h-11 items-center rounded-lg border-l-4 px-4 text-sm font-bold {{ request()->routeIs('office.opportunities.*') ? 'border-brand-blue bg-blue-50 text-brand-blue-dark' : 'border-transparent text-slate-600 hover:bg-slate-50' }}">Opportunities</a>
                @endif
                @if ($activeMembership->hasCapability('quotes.approve'))
                    <a href="{{ route('office.quote-approvals.index') }}" @if(request()->routeIs('office.quote-approvals.*')) aria-current="page" @endif class="mt-1 flex min-h-11 items-center rounded-lg border-l-4 px-4 text-sm font-bold {{ request()->routeIs('office.quote-approvals.*') ? 'border-brand-blue bg-blue-50 text-brand-blue-dark' : 'border-transparent text-slate-600 hover:bg-slate-50' }}">Quote Approvals</a>
                @endif
                ...
                @endif
                @if ($activeMembership->hasCapability('organization.settings.manage') || $activeMembership->hasCapability('billing.settings.manage') || $activeMembership->hasCapability('payments.view') || $activeMembership->hasCapability('opportunities.admin') || $activeMembership->hasCapability('proposal.templates.manage'))
        ...
        <div>
            ...
            <nav class="office-mobile-primary-nav flex gap-2 overflow-x-auto border-b border-slate-200 bg-white px-4 py-2 lg:hidden" aria-label="Office mobile">
                    ...
                    @endif
                    @if($activeMembership->hasCapability('projects.view'))<a href="{{ route('office.projects.index') }}" class="inline-flex min-h-11 shrink-0 items-center rounded-lg px-3 text-sm font-bold {{ request()->routeIs('office.projects.*') ? 'bg-blue-50 text-brand-blue-dark' : 'text-slate-600' }}">Projects</a>@endif
                    @if($activeMembership->hasCapability('opportunities.view'))<a href="{{ route('office.opportunities.index') }}" class="inline-flex min-h-11 shrink-0 items-center rounded-lg px-3 text-sm font-bold {{ request()->routeIs('office.opportunities.*') ? 'bg-blue-50 text-brand-blue-dark' : 'text-slate-600' }}">Opportunities</a>@endif
                    @if($activeMembership->hasCapability('quotes.approve'))<a href="{{ route('office.quote-approvals.index') }}" class="inline-flex min-h-11 shrink-0 items-center rounded-lg px-3 text-sm font-bold {{ request()->routeIs('office.quote-approvals.*') ? 'bg-blue-50 text-brand-blue-dark' : 'text-slate-600' }}">Approvals</a>@endif
