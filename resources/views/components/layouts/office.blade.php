    ...
    <div class="min-h-screen lg:grid lg:grid-cols-[248px_minmax(0,1fr)]">
        <aside class="hidden border-r border-slate-200 bg-white p-5 lg:flex lg:flex-col">
            ...
            <nav class="mt-3" aria-label="Office">
                ...
                @endif
                @if ($activeMembership->hasCapability('catalog.view'))
                    <a href="{{ route('office.catalog.services.index') }}" @if(request()->routeIs('office.catalog.*')) aria-current="page" @endif class="mt-1 flex min-h-11 items-center rounded-lg border-l-4 px-4 text-sm font-bold {{ request()->routeIs('office.catalog.*') ? 'border-brand-blue bg-blue-50 text-brand-blue-dark' : 'border-transparent text-slate-600 hover:bg-slate-50' }}">Catalog</a>
                @endif
        ...
        <div>
            ...
            <nav class="office-mobile-primary-nav flex gap-2 overflow-x-auto border-b border-slate-200 bg-white px-4 py-2 lg:hidden" aria-label="Office mobile">
                    ...
                    @if($activeMembership->hasCapability('catalog.view'))<a href="{{ route('office.catalog.services.index') }}" class="inline-flex min-h-11 shrink-0 items-center rounded-lg px-3 text-sm font-bold {{ request()->routeIs('office.catalog.*') ? 'bg-blue-50 text-brand-blue-dark' : 'text-slate-600' }}">Catalog</a>@endif
