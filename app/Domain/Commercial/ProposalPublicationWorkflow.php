...
final class ProposalPublicationWorkflow
{
    ...
    private function snapshot(CommercialRevision $revision, ProposalTemplate $template): array
    {
        ...
        return [
            ...
            'sections' => $revision->sections->where('customer_visible', true)->map->only(['heading', 'body', 'sort_order'])->values()->all(),
            'terms' => ['name' => $revision->terms_name_snapshot, 'version' => $revision->terms_version_snapshot, 'body' => $revision->terms_body_snapshot],
            'lines' => $revision->lines->map(fn ($line) => ['id' => $line->id, 'type' => $line->line_type, 'description' => $line->description, 'customer_description' => $line->customer_description, 'quantity_millis' => $line->quantity_millis, 'unit_name' => $line->unit_name, 'optional' => $line->optional, 'included' => $line->included, 'taxable' => $line->taxable, 'gross_sell_cents' => $line->gross_sell_cents, 'discount_cents' => $line->line_discount_cents + $line->quote_discount_cents, 'tax_cents' => $line->tax_cents, 'total_cents' => $line->total_cents, 'location' => $line->location?->name, 'system' => $line->system?->name, 'components' => $line->components->where('customer_visible', true)->map->only(['name', 'quantity_millis', 'unit_name'])->values()->all()])->all(),
            'milestones' => $revision->paymentMilestones->map->only(['name', 'amount_type', 'amount_value', 'allocated_cents', 'is_balancing', 'sort_order'])->all(),
