...
final class ProposalPublicationWorkflow
{
    ...
    public function publish(CommercialRevision $revision, ProposalTemplate $template, User $actor, array $data): ProposalPublication
    {
        return DB::transaction(function () use ($revision, $template, $actor, $data): ProposalPublication {
            ...
            $publication = ProposalPublication::query()->create([
                ...
                'tax_cents' => $revision->tax_total_cents, 'total_cents' => $revision->total_cents,
                'change_order_delta_cents' => $revision->change_order_delta_cents, 'resulting_project_total_cents' => $revision->resulting_project_total_cents,
                'acceptance_enabled' => $acceptanceEnabled,
            ...
            $this->audit->record($subject->organization, $actor, $revision->document->document_type === 'change_order' ? 'change_order.publication_created' : 'proposal.publication_created', $subject, ['commercial_document_id' => $revision->commercial_document_id, 'revision_id' => $revision->id, 'publication_id' => $publication->id, 'template_id' => $template->id, 'publication_hash' => $hash, 'total_cents' => $revision->total_cents, 'change_order_delta_cents' => $revision->change_order_delta_cents]);
    ...
    public function addRecipient(ProposalPublication $publication, User $actor, string $email, ?string $name): array
    {
        return DB::transaction(function () use ($publication, $actor, $email, $name): array {
            ...
            $subject = $publication->revision->document->auditSubject();
            $this->audit->record($subject->organization, $actor, $publication->revision->document->document_type === 'change_order' ? 'change_order.recipient_created' : 'proposal.recipient_created', $subject, ['publication_id' => $publication->id, 'recipient_id' => $recipient->id]);
    ...
    public function addShareLink(ProposalPublication $publication, User $actor, ?string $label): array
    {
        return DB::transaction(function () use ($publication, $actor, $label): array {
            ...
            if ($publication->revision->document->document_type === 'quote') {
                $this->opportunities->presented($publication->revision->document->opportunity, $actor, $publication->id);
            }
            $subject = $publication->revision->document->auditSubject();
            $this->audit->record($subject->organization, $actor, $publication->revision->document->document_type === 'change_order' ? 'change_order.share_link_created' : 'proposal.share_link_created', $subject, ['publication_id' => $publication->id, 'share_link_id' => $link->id]);
    ...
    public function withdraw(ProposalPublication $publication, User $actor): ProposalPublication
    {
        return DB::transaction(function () use ($publication, $actor): ProposalPublication {
            ...
            $subject = $publication->revision->document->auditSubject();
            $this->audit->record($subject->organization, $actor, $publication->revision->document->document_type === 'change_order' ? 'change_order.publication_withdrawn' : 'proposal.publication_withdrawn', $subject, ['publication_id' => $publication->id, 'revision_id' => $publication->commercial_revision_id]);
    ...
    private function snapshot(CommercialRevision $revision, ProposalTemplate $template): array
    {
        ...
        return [
            'schema_version' => 1,
            'document' => ['number' => $revision->displayNumber(), 'type' => $revision->document->document_type, 'title' => $revision->document->title, 'currency' => $revision->currency, 'project_id' => $revision->document->project_id],
            ...
            'lines' => $revision->lines->map(fn ($line) => ['id' => $line->id, 'type' => $line->line_type, 'change_effect' => $line->change_effect, 'substitution_group' => $line->substitution_group, 'catalog_product_id' => $line->catalog_product_id, 'catalog_service_id' => $line->catalog_service_id, 'catalog_service_variant_id' => $line->catalog_service_variant_id, 'catalog_package_id' => $line->catalog_package_id, 'catalog_unit_sell_cents' => $line->catalog_unit_sell_cents, 'effective_unit_sell_cents' => $line->effective_unit_sell_cents, 'discount_type' => $line->discount_type, 'discount_value' => $line->discount_value, 'description' => $line->description, 'customer_description' => $line->customer_description, 'quantity_millis' => $line->quantity_millis, 'unit_name' => $line->unit_name, 'optional' => $line->optional, 'included' => $line->included, 'taxable' => $line->taxable, 'gross_sell_cents' => $line->gross_sell_cents, 'discount_cents' => $line->line_discount_cents + $line->quote_discount_cents, 'tax_cents' => $line->tax_cents, 'total_cents' => $line->total_cents, 'location' => $line->location?->name, 'system' => $line->system?->name, 'phase' => $line->phase?->name, 'components' => $line->components->where('customer_visible', true)->map->only(['name', 'quantity_millis', 'unit_name'])->values()->all(), 'conversion_components' => $line->components->map->only(['id', 'component_type', 'catalog_product_id', 'catalog_service_id', 'name', 'quantity_millis', 'waste_basis_points', 'unit_name'])->values()->all()])->all(),
            ...
            'totals' => ['subtotal_cents' => $revision->subtotal_cents, 'line_discount_cents' => $revision->line_discount_total_cents, 'quote_discount_cents' => $revision->quote_discount_total_cents, 'tax_cents' => $revision->tax_total_cents, 'total_cents' => $revision->total_cents, 'change_order_delta_cents' => $revision->change_order_delta_cents, 'resulting_project_total_cents' => $revision->resulting_project_total_cents],
